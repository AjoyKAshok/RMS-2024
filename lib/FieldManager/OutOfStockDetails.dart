...
class OutOfStockDetails extends StatefulWidget {
  const OutOfStockDetails({super.key});

  @override
  State<OutOfStockDetails> createState() => _OutOfStockDetailsState();
}

class _OutOfStockDetailsState extends State<OutOfStockDetails> {
  int i = 0;
  Data? data;
  int lengthlist = 0;
  String userName = "";
  ...
  Map<String, int> oosCountMap = {};
  late Stream<Map<String, int>> oosCountStream;
  late StreamSubscription<Map<String, int>> oosCountSubscription;
  ...
  initState() {
    ...
    _outlet();
    loader();
    // Initialize and listen to the OOS count stream
    oosCountStream = getOOSCountStream();
    oosCountSubscription = oosCountStream.listen((event) {
      setState(() {
        oosCountMap = event;
      });
    });
  ...
  void dispose() {
    ...
    oosCountSubscription.cancel();
    super.dispose();
  }

  // Add this new method to fetch OOS count
  Future<int> fetchOOSCount(String storeCode) async {
    // Implement your API call here to fetch the latest OOS count for the given store
    // This is a placeholder implementation
    await Future.delayed(Duration(seconds: 1)); // Simulating network delay
    return Future.value(DateTime.now().second % 10); // Random count for demonstration
  }

  // Add this method to create a stream of OOS counts
  Stream<Map<String, int>> getOOSCountStream() async* {
    while (true) {
      Map<String, int> newCounts = {};
      for (var store in customList) {
        String storeCode = store['store_code'];
        int count = await fetchOOSCount(storeCode);
        newCounts[storeCode] = count;
      }
      yield newCounts;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            const Text("Out Of Stock Details"),
            userName.isNotEmpty
                ? Text(
                      ...
                      ? queryStarted
                                ...
                                Expanded(
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      String? storeCode = filteredStores[index]['store_code'];
                                      return Padding(
                                        padding:
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                String? storeCode = _outletModel!.data![index].store![0].storeCode;
                                return Padding(
                                      ...
                                      child: Padding(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            // Text(
                                            //   "Address: ${_outletModel!.data![index]
                                            //       .store![0].address
                                            //       .toString()}",
                                            //   style: const TextStyle(
                                            //       color: Colors.black,
